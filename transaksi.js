...
router.post("/pelanggaran_siswa", (req, res) => {
    ...
    // parse to JSON
    let pelanggaran = JSON.parse(req.body.pelanggaran)//parse digunakan menangkan JSON //JSON=nama file==mengubah tipe data
    ...
    db.query(sql, data, (error, result) => {
        ...
        if (error) {
            ...
            let data = []
            for (let index = 0; index < pelanggaran.length; index++) { //pelangggaran.length untuk mengitung panjangnya data di array
                data.push([
